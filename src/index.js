...
class Dir extends Object {
     ...
     * @param {Number = Infinity} depthLimit Maximal depth limit of recurrency
     * @returns {Dir}
     */
    static from(path, depthLimit = Infinity) {
        if (depthLimit-- === 0) return path;
        const dir = new Dir(path);
        for (let child of FS.readdirSync(path)) dir.pushChild(`${path}/${child}`, depthLimit);
        return dir
    }
    ...
    *[Symbol.iterator]() {
     ...
     * @param {Number} depthLimit Depth limit
     */
    pushChild(path, depthLimit) {
        const base = path.replace(REGEX_BASE, ''),
            name = (base.replace(REGEX_NAME, '') || base).replace(/\./gm, '_');
        if (FS.lstatSync(path).isDirectory()) this[name] = Dir.from(path, depthLimit);
        else this[name] = path;
    };
